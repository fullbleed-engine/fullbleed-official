from __future__ import annotations

import json
from pathlib import Path

import pytest

import fullbleed
from fullbleed.audit_prototype import prototype_verify_accessibility


ROOT = Path(__file__).resolve().parents[1]
SPECS = ROOT / "docs" / "specs"


def _require_pdf_engine() -> None:
    if not hasattr(fullbleed, "PdfEngine"):
        pytest.skip("fullbleed native extension is not available in this test environment")


def _write(path: Path, text: str) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(text, encoding="utf-8")
    return path


def _render_preview_png(engine: "fullbleed.PdfEngine", html: str, css: str, out_dir: Path, stem: str = "preview") -> Path:
    out_dir.mkdir(parents=True, exist_ok=True)
    if hasattr(engine, "render_image_pages_to_dir"):
        paths = list(engine.render_image_pages_to_dir(html, css, str(out_dir), 144, stem) or [])
        if paths:
            return Path(paths[0])
    if hasattr(engine, "render_image_pages"):
        imgs = list(engine.render_image_pages(html, css, 144) or [])
        if imgs:
            path = out_dir / f"{stem}_page1.png"
            path.write_bytes(imgs[0])
            return path
    pytest.skip("engine PNG render preview API not available")


@pytest.fixture(scope="module")
def jsonschema_module():
    return pytest.importorskip("jsonschema")


def test_pdf_engine_verify_accessibility_artifacts_emits_schema_valid_report(
    tmp_path: Path, jsonschema_module
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "doc.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Verifier Doc</title>"
            "<link rel='stylesheet' href='doc.css'></head><body><main id='root'>"
            "<p id='p1' aria-labelledby='label1'>Hello</p>"
            "<span id='label1'>Greeting</span>"
            "</main></body></html>"
        ),
    )
    css = _write(tmp_path / "doc.css", "body{font-family:Helvetica}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Verifier Doc")
    report = engine.verify_accessibility_artifacts(
        str(html),
        str(css),
        profile="strict",
        mode="error",
    )

    schema = json.loads(
        (SPECS / "fullbleed.a11y.verify.v1.schema.json").read_text(encoding="utf-8")
    )
    jsonschema_module.Draft202012Validator(schema).validate(report)

    assert report["schema"] == "fullbleed.a11y.verify.v1"
    assert report["gate"]["ok"] is True
    assert report["conformance_status"]["status"] == "pass_machine_subset"
    assert report["target"]["html_path"] == str(html)
    assert report["target"]["css_path"] == str(css)
    assert report["artifacts"]["css_linked"] is True
    assert report["coverage"]["wcag20aa"]["registry_id"] == "wcag20aa_registry.v1"
    assert report["coverage"]["wcag20aa"]["total_entries"] == 43
    assert report["coverage"]["section508"]["registry_id"] == "section508_html_registry.v1"
    assert report["coverage"]["section508"]["inherited_wcag_entries_total"] == 43
    assert report["observability"]["dedup_event_count"] == 0
    assert report["observability"]["reported_finding_count"] == len(report["findings"])
    assert report["observability"]["stage_counts"]["post-emit"] >= 1
    assert report["observability"]["correlation_index"] == []
    assert report["wcag20aa_claim_readiness"]["target"] == "wcag20aa"
    assert report["wcag20aa_claim_readiness"]["claim_ready"] is False
    assert any(
        f["rule_id"] == "fb.a11y.claim.wcag20aa_level_readiness"
        for f in report["findings"]
    )
    assert any(
        row["pack_id"] == "wcag20aa.implemented_map.v1"
        for row in report["coverage"]["rule_pack_coverage"]
    )
    assert any(
        row["pack_id"] == "section508_html.implemented_map.v1"
        for row in report["coverage"]["rule_pack_coverage"]
    )
    assert report["tooling"]["audit_contract_id"] == "fullbleed.audit_contract"
    assert report["tooling"]["audit_contract_version"] == "1"
    assert report["tooling"]["audit_contract_fingerprint"].startswith("sha256:")
    assert report["tooling"]["section508_html_registry_hash"].startswith("sha256:")


def test_pdf_engine_verify_accessibility_artifacts_surfaces_pagination_trace_summary(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "doc.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Verifier Pagination</title></head>"
            "<body><main id='root'><p>Hello</p></main></body></html>"
        ),
    )
    css = _write(tmp_path / "doc.css", "body{font-family:Helvetica}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Verifier Pagination")
    report = engine.verify_accessibility_artifacts(
        str(html),
        str(css),
        profile="strict",
        mode="error",
        pagination_trace_summary={
            "page_count": 1,
            "overflow_event_count": 2,
            "flowable_overlap_count": 1,
            "text_overlap_count": 3,
            "transition_count": 4,
        },
    )

    assert report["pagination_trace_summary"]["overflow_event_count"] == 2
    signals = report["observability"]["signal_counts"]
    assert signals["pagination_page_count"] == 1
    assert signals["pagination_overflow_event_count"] == 2
    assert signals["pagination_flowable_overlap_count"] == 1
    assert signals["pagination_text_overlap_count"] == 3
    assert signals["pagination_transition_count"] == 4


def test_pdf_engine_verify_accessibility_artifacts_fails_fast_for_ids_and_idrefs(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "bad.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Bad</title></head><body>"
            "<main id='m1'>"
            "<div id='dup'></div><div id='dup' aria-describedby='missing-id'></div>"
            "</main></body></html>"
        ),
    )
    css = _write(tmp_path / "bad.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Bad")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    assert report["gate"]["ok"] is False
    failed = set(report["gate"]["failed_rule_ids"])
    assert "fb.a11y.ids.duplicate_id" in failed
    assert "fb.a11y.aria.reference_target_exists" in failed
    assert any(
        f["rule_id"] == "fb.a11y.ids.duplicate_id" and f["verdict"] == "fail"
        for f in report["findings"]
    )
    assert any(
        f["rule_id"] == "fb.a11y.aria.reference_target_exists" and f["verdict"] == "fail"
        for f in report["findings"]
    )


def test_engine_verifier_signature_semantics_fail_is_warn_level_gate(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "sig.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Sig</title></head><body>"
            "<main><p>Signature of Declarant</p></main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "sig.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Sig")
    report = engine.verify_accessibility_artifacts(
        str(html),
        str(css),
        profile="cav",
        mode="error",
    )

    sig_rows = [
        f
        for f in report["findings"]
        if f["rule_id"] == "fb.a11y.signatures.text_semantics_present"
    ]
    assert sig_rows
    assert any(f["verdict"] == "fail" for f in sig_rows)
    assert report["gate"]["ok"] is True
    assert report["gate"]["error_count"] == 0
    assert "fb.a11y.signatures.text_semantics_present" not in set(
        report["gate"]["failed_rule_ids"]
    )
    assert report["gate"]["warn_count"] >= 1


def test_engine_verifier_matches_prototype_for_shared_core_rules(tmp_path: Path) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "doc.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Parity Doc</title></head>"
            "<body><main id='root'><div id='alpha' aria-labelledby='missing'>x</div></main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "doc.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Parity Doc")
    engine_report = engine.verify_accessibility_artifacts(
        str(html),
        str(css),
        profile="strict",
        mode="error",
    )
    prototype_report = prototype_verify_accessibility(
        html_path=html,
        css_path=css,
        profile="strict",
        mode="error",
        expected_lang="en-US",
        expected_title="Parity Doc",
        generated_at="2026-02-24T00:00:00Z",
    )

    shared_rule_ids = {
        "fb.a11y.html.lang_present_valid",
        "fb.a11y.language.parts_declared_valid_seed",
        "fb.a11y.html.title_present_nonempty",
        "fb.a11y.structure.single_main",
        "fb.a11y.headings_labels.present_nonempty",
        "fb.a11y.images.alt_or_decorative",
        "fb.a11y.forms.labels_or_instructions_present",
        "fb.a11y.forms.error_identification_present",
        "fb.a11y.focus.order_seed",
        "fb.a11y.links.purpose_in_context",
        "fb.a11y.instructions.sensory_characteristics_seed",
        "fb.a11y.focus.visible_seed",
        "fb.a11y.claim.non_interference_seed",
        "fb.a11y.claim.complete_processes_scope_seed",
        "fb.a11y.keyboard.operable_seed",
        "fb.a11y.keyboard.no_trap_seed",
        "fb.a11y.forms.on_input_behavior_seed",
        "fb.a11y.focus.on_focus_behavior_seed",
        "fb.a11y.timing.adjustable_seed",
        "fb.a11y.timing.pause_stop_hide_seed",
        "fb.a11y.seizures.three_flashes_seed",
        "fb.a11y.audio.control_seed",
        "fb.a11y.color.use_of_color_seed",
        "fb.a11y.text.resize_seed",
        "fb.a11y.images.of_text_seed",
        "fb.a11y.media.prerecorded_audio_video_alternative_seed",
        "fb.a11y.media.prerecorded_captions_seed",
        "fb.a11y.media.prerecorded_audio_description_or_media_alternative_seed",
        "fb.a11y.media.live_captions_seed",
        "fb.a11y.media.prerecorded_audio_description_seed",
        "fb.a11y.sequence.meaningful_sequence_seed",
        "fb.a11y.forms.error_suggestion_seed",
        "fb.a11y.forms.error_prevention_legal_financial_data_seed",
        "fb.a11y.navigation.multiple_ways_seed",
        "fb.a11y.navigation.consistent_navigation_seed",
        "fb.a11y.claim.accessibility_supported_technologies_seed",
        "fb.a11y.identification.consistent_identification_seed",
        "fb.a11y.claim.section508.public_facing_content_applicability_seed",
        "fb.a11y.claim.section508.official_communications_applicability_seed",
        "fb.a11y.claim.section508.nara_exception_applicability_seed",
        "fb.a11y.claim.section508.non_web_document_exceptions_html_seed",
        "fb.a11y.claim.wcag20aa_level_readiness",
        "fb.a11y.ids.duplicate_id",
        "fb.a11y.aria.reference_target_exists",
    }

    def _verdicts(report: dict) -> dict[str, list[str]]:
        out: dict[str, list[str]] = {}
        for finding in report["findings"]:
            rid = finding["rule_id"]
            if rid not in shared_rule_ids:
                continue
            out.setdefault(rid, []).append(finding["verdict"])
        return out

    assert _verdicts(engine_report) == _verdicts(prototype_report)


def test_engine_verifier_flags_empty_headings_labels_and_unlabeled_regions(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "doc.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Headings</title></head><body>"
            "<main><h1></h1><label> </label><section role='region'></section></main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "doc.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Headings")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    matches = [
        f
        for f in report["findings"]
        if f["rule_id"] == "fb.a11y.headings_labels.present_nonempty"
    ]
    assert matches
    assert any(f["verdict"] == "fail" for f in matches)
    assert any(
        f["evidence"][0]["values"]["empty_heading_count"] == "1"
        and f["evidence"][0]["values"]["empty_label_count"] == "1"
        and f["evidence"][0]["values"]["unlabeled_region_count"] == "1"
        for f in matches
    )


def test_engine_verifier_flags_image_missing_alt_and_title_only(tmp_path: Path) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "images.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Images</title></head><body>"
            "<main>"
            "<img src='a.png'>"
            "<img src='b.png' title='Chart image'>"
            "</main></body></html>"
        ),
    )
    css = _write(tmp_path / "images.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Images")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    matches = [
        f
        for f in report["findings"]
        if f["rule_id"] == "fb.a11y.images.alt_or_decorative"
    ]
    assert matches
    assert any(f["verdict"] == "fail" for f in matches)
    assert any(
        f["evidence"][0]["values"]["image_missing_alt_count"] == "1"
        and f["evidence"][0]["values"]["image_title_only_count"] == "1"
        for f in matches
    )


def test_engine_verifier_emits_figure_alt_budget_redundancy_and_effective_text_rules(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    long_alt = (
        "Scanned warranty deed certification block showing county certificate text, "
        "signatures, notary references, and recording stamp details for Escambia County Florida."
    )
    html = _write(
        tmp_path / "figure-rules.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Figure Rules</title></head><body>"
            "<main>"
            f"<figure><img src='a.png' alt='{long_alt}'><figcaption>{long_alt}</figcaption></figure>"
            "<figure><img src='b.png'></figure>"
            "</main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "figure-rules.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Figure Rules")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    by_rule = {f["rule_id"]: f for f in report["findings"]}
    alt_budget = by_rule["fb.a11y.figure.alt_length_budget_seed"]
    caption_dupe = by_rule["fb.a11y.figure.caption_redundancy_seed"]
    missing_effective = by_rule["fb.a11y.figure.missing_effective_text_seed"]

    assert alt_budget["verdict"] == "warn"
    assert caption_dupe["verdict"] == "warn"
    assert missing_effective["verdict"] == "fail"

    alt_values = alt_budget["evidence"][0]["values"]
    assert alt_values["figure_informative_count"] == "2"
    assert alt_values["figure_alt_length_budget"] == "150"
    assert alt_values["figure_alt_over_budget_count"] == "1"

    dupe_values = caption_dupe["evidence"][0]["values"]
    assert dupe_values["figure_caption_redundancy_count"] == "1"
    assert float(dupe_values["figure_max_caption_similarity"]) >= 0.8

    missing_values = missing_effective["evidence"][0]["values"]
    assert missing_values["figure_missing_effective_text_count"] == "1"


def test_engine_verifier_emits_dl_fragmentation_and_group_consistency_rules(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "dl-rules.html",
        (
            "<!doctype html><html lang='en-US'><head><title>DL Rules</title></head><body>"
            "<main><section>"
            "<dl><dt>Label A</dt><dd>Value A</dd></dl>"
            "<dl><dt>Label B</dt><dd>Value B</dd></dl>"
            "</section></main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "dl-rules.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="DL Rules")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    by_rule = {f["rule_id"]: f for f in report["findings"]}
    fragmented = by_rule["fb.a11y.dl.fragmentation_seed"]
    grouped = by_rule["fb.a11y.dl.group_consistency_seed"]

    assert fragmented["verdict"] == "warn"
    assert grouped["verdict"] == "warn"
    assert fragmented["evidence"][0]["values"]["dl_block_count"] == "2"
    assert fragmented["evidence"][0]["values"]["dl_fragmentation_count"] == "1"
    assert grouped["evidence"][0]["values"]["dl_group_consistency_count"] == "1"
    assert report["observability"]["signal_counts"]["dl_fragmentation_count"] == 1


def test_engine_verifier_emits_redundant_aria_native_rules(tmp_path: Path) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "aria-native-rules.html",
        (
            "<!doctype html><html lang='en-US'><head><title>ARIA Native Rules</title></head><body>"
            "<main>"
            "<nav role='navigation'>Primary</nav>"
            "<button role='button'>Save</button>"
            "<input required aria-required='true'>"
            "<button disabled aria-disabled='true'>Disabled</button>"
            "</main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "aria-native-rules.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="ARIA Native Rules")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    by_rule = {f["rule_id"]: f for f in report["findings"]}
    role_row = by_rule["fb.a11y.aria.redundant_role_native_seed"]
    state_row = by_rule["fb.a11y.aria.redundant_state_native_seed"]

    assert role_row["verdict"] == "warn"
    assert state_row["verdict"] == "warn"
    assert int(role_row["evidence"][0]["values"]["redundant_role_native_count"]) >= 1
    assert int(state_row["evidence"][0]["values"]["redundant_state_native_count"]) >= 1
    assert report["observability"]["signal_counts"]["redundant_aria_count"] >= 2


def test_engine_verifier_flags_unlabeled_form_controls(tmp_path: Path) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "forms.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Forms</title></head><body><main>"
            "<label for='good'>Full Name</label><input id='good'>"
            "<input id='bad'>"
            "<textarea aria-label='Notes'></textarea>"
            "</main></body></html>"
        ),
    )
    css = _write(tmp_path / "forms.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Forms")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    rows = [
        f
        for f in report["findings"]
        if f["rule_id"] == "fb.a11y.forms.labels_or_instructions_present"
    ]
    assert rows
    assert any(f["verdict"] == "fail" for f in rows)
    assert any(
        f["evidence"][0]["values"]["form_control_count"] == "3"
        and f["evidence"][0]["values"]["unlabeled_form_control_count"] == "1"
        for f in rows
    )


def test_engine_verifier_flags_invalid_controls_without_error_identification(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "errors.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Errors</title></head><body><main>"
            "<input id='ok' aria-invalid='true' aria-describedby='err-ok'>"
            "<div id='err-ok'>Email is required</div>"
            "<input id='bad' aria-invalid='true'>"
            "</main></body></html>"
        ),
    )
    css = _write(tmp_path / "errors.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Errors")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    rows = [
        f
        for f in report["findings"]
        if f["rule_id"] == "fb.a11y.forms.error_identification_present"
    ]
    assert rows
    assert any(f["verdict"] == "fail" for f in rows)
    assert any(
        f["evidence"][0]["values"]["invalid_form_control_count"] == "2"
        and f["evidence"][0]["values"]["unidentified_error_form_control_count"] == "1"
        for f in rows
    )


def test_engine_verifier_detects_unnamed_and_generic_link_purpose_signals(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "links.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Links</title></head><body><main>"
            "<a href='/empty'> </a>"
            "<a href='/generic'>Click here</a>"
            "<a href='/good'>Marriage record details</a>"
            "</main></body></html>"
        ),
    )
    css = _write(tmp_path / "links.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Links")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    rows = [f for f in report["findings"] if f["rule_id"] == "fb.a11y.links.purpose_in_context"]
    assert rows
    assert any(f["verdict"] == "fail" for f in rows)
    assert any(
        f["evidence"][0]["values"]["link_count"] == "3"
        and f["evidence"][0]["values"]["unnamed_link_count"] == "1"
        and f["evidence"][0]["values"]["generic_link_text_count"] == "1"
        for f in rows
    )


def test_engine_verifier_warns_on_sensory_characteristics_instruction_phrases(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "sensory.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Sensory</title></head><body><main>"
            "<p>See below for the details and sign on the right.</p>"
            "</main></body></html>"
        ),
    )
    css = _write(tmp_path / "sensory.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Sensory")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    rows = [
        f
        for f in report["findings"]
        if f["rule_id"] == "fb.a11y.instructions.sensory_characteristics_seed"
    ]
    assert rows
    assert any(f["verdict"] == "warn" for f in rows)
    assert any(
        f["evidence"][0]["values"]["sensory_phrase_hit_count"] == "2"
        and "see below" in f["evidence"][0]["values"]["sensory_phrase_hits"]
        and "on the right" in f["evidence"][0]["values"]["sensory_phrase_hits"]
        for f in rows
    )


def test_engine_verifier_flags_invalid_language_of_parts_declarations(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "lang-parts.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Lang Parts</title></head><body>"
            "<main><p><span lang='es'>Hola</span> <span lang='-bad'>mundo</span></p></main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "lang-parts.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Lang Parts")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    rows = [
        f
        for f in report["findings"]
        if f["rule_id"] == "fb.a11y.language.parts_declared_valid_seed"
    ]
    assert rows
    assert any(f["verdict"] == "fail" for f in rows)
    assert any(
        f["evidence"][0]["values"]["part_lang_attr_count"] == "2"
        and f["evidence"][0]["values"]["invalid_part_lang_attr_count"] == "1"
        for f in rows
    )


def test_engine_verifier_warns_on_non_interference_risk_signals(tmp_path: Path) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "active.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Active</title>"
            "<meta http-equiv='refresh' content='5'></head><body>"
            "<main><script>console.log('x')</script><div onclick='x()'>x</div></main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "active.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Active")
    report = engine.verify_accessibility_artifacts(str(html), str(css), mode="error")

    rows = [f for f in report["findings"] if f["rule_id"] == "fb.a11y.claim.non_interference_seed"]
    assert rows
    assert any(f["verdict"] == "warn" for f in rows)
    assert any(
        f["evidence"][0]["values"]["script_element_count"] == "1"
        and f["evidence"][0]["values"]["inline_event_handler_attr_count"] == "1"
        and f["evidence"][0]["values"]["meta_refresh_count"] == "1"
        for f in rows
    )
    tech_rows = [
        f
        for f in report["findings"]
        if f["rule_id"] == "fb.a11y.claim.accessibility_supported_technologies_seed"
    ]
    assert tech_rows
    assert any(f["verdict"] == "warn" for f in tech_rows)
    assert any(
        f["evidence"][0]["values"]["script_element_count"] == "1"
        and f["evidence"][0]["values"]["meta_refresh_count"] == "1"
        for f in tech_rows
    )


def test_engine_verifier_marks_complete_processes_scope_seed_manual_for_transactional_profile(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "txn.html",
        "<!doctype html><html lang='en-US'><head><title>Txn</title></head><body><main></main></body></html>",
    )
    css = _write(tmp_path / "txn.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Txn")
    report = engine.verify_accessibility_artifacts(
        str(html), str(css), profile="transactional", mode="error"
    )

    rows = [
        f
        for f in report["findings"]
        if f["rule_id"] == "fb.a11y.claim.complete_processes_scope_seed"
    ]
    assert rows
    assert any(f["verdict"] == "manual_needed" for f in rows)
    assert any(f["applicability"] == "applicable" for f in rows)
    assert any(
        f["evidence"][0]["values"]["profile"] == "transactional"
        and f["evidence"][0]["values"]["process_scope_declared"] == "false"
        for f in rows
    )


def test_engine_verifier_emits_section508_e205_claim_seed_rules(tmp_path: Path) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "s508.html",
        "<!doctype html><html lang='en-US'><head><title>S508</title></head><body><main></main></body></html>",
    )
    css = _write(tmp_path / "s508.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="S508")
    report = engine.verify_accessibility_artifacts(str(html), str(css), profile="strict", mode="error")

    rows = {f["rule_id"]: f for f in report["findings"]}
    assert rows["fb.a11y.claim.section508.public_facing_content_applicability_seed"]["verdict"] == "manual_needed"
    assert rows["fb.a11y.claim.section508.public_facing_content_applicability_seed"]["applicability"] == "applicable"
    assert rows["fb.a11y.claim.section508.official_communications_applicability_seed"]["verdict"] == "manual_needed"
    assert rows["fb.a11y.claim.section508.nara_exception_applicability_seed"]["verdict"] == "manual_needed"
    non_web = rows["fb.a11y.claim.section508.non_web_document_exceptions_html_seed"]
    assert non_web["verdict"] == "not_applicable"
    assert non_web["applicability"] == "not_applicable"
    assert any(
        ev["values"].get("delivery_target") == "html"
        for ev in non_web.get("evidence", [])
    )


def test_engine_verifier_claim_evidence_can_satisfy_claim_seed_rules(tmp_path: Path) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "claim.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Claim</title></head><body>"
            "<main><a href='#x'>Go</a><input aria-label='Name'></main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "claim.css", "body{}")
    claim_evidence = {
        "technology_support": {
            "assessed": True,
            "basis_recorded": True,
        },
        "wcag20": {
            "keyboard_assessed": True,
            "keyboard_basis_recorded": True,
            "keyboard_trap_assessed": True,
            "keyboard_trap_basis_recorded": True,
            "on_input_assessed": True,
            "on_input_basis_recorded": True,
            "on_focus_assessed": True,
            "on_focus_basis_recorded": True,
            "timing_adjustable_scope_declared": True,
            "timing_adjustable_assessed": True,
            "timing_adjustable_basis_recorded": True,
            "pause_stop_hide_scope_declared": True,
            "pause_stop_hide_assessed": True,
            "pause_stop_hide_basis_recorded": True,
            "three_flashes_scope_declared": True,
            "three_flashes_assessed": True,
            "three_flashes_basis_recorded": True,
            "audio_control_scope_declared": True,
            "audio_control_assessed": True,
            "audio_control_basis_recorded": True,
            "use_of_color_scope_declared": True,
            "use_of_color_assessed": True,
            "use_of_color_basis_recorded": True,
            "resize_text_scope_declared": True,
            "resize_text_assessed": True,
            "resize_text_basis_recorded": True,
            "images_of_text_scope_declared": True,
            "images_of_text_assessed": True,
            "images_of_text_basis_recorded": True,
            "prerecorded_av_alternative_scope_declared": True,
            "prerecorded_av_alternative_assessed": True,
            "prerecorded_av_alternative_basis_recorded": True,
            "prerecorded_captions_scope_declared": True,
            "prerecorded_captions_assessed": True,
            "prerecorded_captions_basis_recorded": True,
            "prerecorded_audio_description_or_media_alternative_scope_declared": True,
            "prerecorded_audio_description_or_media_alternative_assessed": True,
            "prerecorded_audio_description_or_media_alternative_basis_recorded": True,
            "live_captions_scope_declared": True,
            "live_captions_assessed": True,
            "live_captions_basis_recorded": True,
            "prerecorded_audio_description_scope_declared": True,
            "prerecorded_audio_description_assessed": True,
            "prerecorded_audio_description_basis_recorded": True,
            "meaningful_sequence_scope_declared": True,
            "meaningful_sequence_assessed": True,
            "meaningful_sequence_basis_recorded": True,
            "error_suggestion_scope_declared": True,
            "error_suggestion_assessed": True,
            "error_suggestion_basis_recorded": True,
            "error_prevention_scope_declared": True,
            "error_prevention_assessed": True,
            "error_prevention_basis_recorded": True,
            "consistent_identification_assessed": True,
            "consistent_identification_basis_recorded": True,
            "multiple_ways_scope_declared": True,
            "multiple_ways_assessed": True,
            "multiple_ways_basis_recorded": True,
            "consistent_navigation_scope_declared": True,
            "consistent_navigation_assessed": True,
            "consistent_navigation_basis_recorded": True,
        },
        "section508": {
            "scope_declared": True,
            "public_facing_determination_recorded": True,
            "official_communications_determination_recorded": True,
            "nara_exception_determination_recorded": True,
        },
    }

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Claim")
    report = engine.verify_accessibility_artifacts(
        str(html),
        str(css),
        profile="cav",
        mode="error",
        claim_evidence=claim_evidence,
    )

    rows = {f["rule_id"]: f for f in report["findings"]}
    assert rows["fb.a11y.claim.accessibility_supported_technologies_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.identification.consistent_identification_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.keyboard.operable_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.keyboard.no_trap_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.forms.on_input_behavior_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.focus.on_focus_behavior_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.timing.adjustable_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.timing.pause_stop_hide_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.seizures.three_flashes_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.audio.control_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.color.use_of_color_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.text.resize_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.images.of_text_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.media.prerecorded_audio_video_alternative_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.media.prerecorded_captions_seed"]["verdict"] == "pass"
    assert (
        rows["fb.a11y.media.prerecorded_audio_description_or_media_alternative_seed"][
            "verdict"
        ]
        == "pass"
    )
    assert rows["fb.a11y.media.live_captions_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.media.prerecorded_audio_description_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.sequence.meaningful_sequence_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.forms.error_suggestion_seed"]["verdict"] == "pass"
    assert (
        rows["fb.a11y.forms.error_prevention_legal_financial_data_seed"]["verdict"]
        == "pass"
    )
    assert rows["fb.a11y.navigation.multiple_ways_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.navigation.consistent_navigation_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.claim.section508.public_facing_content_applicability_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.claim.section508.official_communications_applicability_seed"]["verdict"] == "pass"
    assert rows["fb.a11y.claim.section508.nara_exception_applicability_seed"]["verdict"] == "pass"
    assert (
        rows["fb.a11y.claim.section508.non_web_document_exceptions_html_seed"]["verdict"]
        == "not_applicable"
    )
    tech_evidence = rows["fb.a11y.claim.accessibility_supported_technologies_seed"]["evidence"][0]["values"]
    assert tech_evidence["technology_support_assessed"] == "true"
    assert tech_evidence["technology_support_basis_recorded"] == "true"
    cid_evidence = rows["fb.a11y.identification.consistent_identification_seed"]["evidence"][0]["values"]
    assert cid_evidence["consistent_identification_assessed"] == "true"
    assert cid_evidence["consistent_identification_basis_recorded"] == "true"
    keyboard_evidence = rows["fb.a11y.keyboard.operable_seed"]["evidence"][0]["values"]
    assert keyboard_evidence["keyboard_assessed"] == "true"
    assert keyboard_evidence["keyboard_basis_recorded"] == "true"
    keyboard_trap_evidence = rows["fb.a11y.keyboard.no_trap_seed"]["evidence"][0]["values"]
    assert keyboard_trap_evidence["keyboard_trap_assessed"] == "true"
    assert keyboard_trap_evidence["keyboard_trap_basis_recorded"] == "true"
    on_input_evidence = rows["fb.a11y.forms.on_input_behavior_seed"]["evidence"][0]["values"]
    assert on_input_evidence["on_input_assessed"] == "true"
    assert on_input_evidence["on_input_basis_recorded"] == "true"
    on_focus_evidence = rows["fb.a11y.focus.on_focus_behavior_seed"]["evidence"][0]["values"]
    assert on_focus_evidence["on_focus_assessed"] == "true"
    assert on_focus_evidence["on_focus_basis_recorded"] == "true"
    timing_evidence = rows["fb.a11y.timing.adjustable_seed"]["evidence"][0]["values"]
    assert timing_evidence["timing_adjustable_scope_declared"] == "true"
    assert timing_evidence["timing_adjustable_assessed"] == "true"
    assert timing_evidence["timing_adjustable_basis_recorded"] == "true"
    pause_stop_hide_evidence = rows["fb.a11y.timing.pause_stop_hide_seed"]["evidence"][0]["values"]
    assert pause_stop_hide_evidence["pause_stop_hide_scope_declared"] == "true"
    assert pause_stop_hide_evidence["pause_stop_hide_assessed"] == "true"
    assert pause_stop_hide_evidence["pause_stop_hide_basis_recorded"] == "true"
    three_flashes_evidence = rows["fb.a11y.seizures.three_flashes_seed"]["evidence"][0]["values"]
    assert three_flashes_evidence["three_flashes_scope_declared"] == "true"
    assert three_flashes_evidence["three_flashes_assessed"] == "true"
    assert three_flashes_evidence["three_flashes_basis_recorded"] == "true"
    audio_control_evidence = rows["fb.a11y.audio.control_seed"]["evidence"][0]["values"]
    assert audio_control_evidence["audio_control_scope_declared"] == "true"
    assert audio_control_evidence["audio_control_assessed"] == "true"
    assert audio_control_evidence["audio_control_basis_recorded"] == "true"
    use_of_color_evidence = rows["fb.a11y.color.use_of_color_seed"]["evidence"][0]["values"]
    assert use_of_color_evidence["use_of_color_scope_declared"] == "true"
    assert use_of_color_evidence["use_of_color_assessed"] == "true"
    assert use_of_color_evidence["use_of_color_basis_recorded"] == "true"
    resize_text_evidence = rows["fb.a11y.text.resize_seed"]["evidence"][0]["values"]
    assert resize_text_evidence["resize_text_scope_declared"] == "true"
    assert resize_text_evidence["resize_text_assessed"] == "true"
    assert resize_text_evidence["resize_text_basis_recorded"] == "true"
    images_of_text_evidence = rows["fb.a11y.images.of_text_seed"]["evidence"][0]["values"]
    assert images_of_text_evidence["images_of_text_scope_declared"] == "true"
    assert images_of_text_evidence["images_of_text_assessed"] == "true"
    assert images_of_text_evidence["images_of_text_basis_recorded"] == "true"
    prerec_av_alt_evidence = rows[
        "fb.a11y.media.prerecorded_audio_video_alternative_seed"
    ]["evidence"][0]["values"]
    assert prerec_av_alt_evidence["prerecorded_av_alternative_scope_declared"] == "true"
    assert prerec_av_alt_evidence["prerecorded_av_alternative_assessed"] == "true"
    assert prerec_av_alt_evidence["prerecorded_av_alternative_basis_recorded"] == "true"
    prerec_captions_evidence = rows["fb.a11y.media.prerecorded_captions_seed"]["evidence"][0]["values"]
    assert prerec_captions_evidence["prerecorded_captions_scope_declared"] == "true"
    assert prerec_captions_evidence["prerecorded_captions_assessed"] == "true"
    assert prerec_captions_evidence["prerecorded_captions_basis_recorded"] == "true"
    prerec_ad_or_alt_evidence = rows[
        "fb.a11y.media.prerecorded_audio_description_or_media_alternative_seed"
    ]["evidence"][0]["values"]
    assert (
        prerec_ad_or_alt_evidence[
            "prerecorded_audio_description_or_media_alternative_scope_declared"
        ]
        == "true"
    )
    assert (
        prerec_ad_or_alt_evidence[
            "prerecorded_audio_description_or_media_alternative_assessed"
        ]
        == "true"
    )
    assert (
        prerec_ad_or_alt_evidence[
            "prerecorded_audio_description_or_media_alternative_basis_recorded"
        ]
        == "true"
    )
    live_captions_evidence = rows["fb.a11y.media.live_captions_seed"]["evidence"][0]["values"]
    assert live_captions_evidence["live_captions_scope_declared"] == "true"
    assert live_captions_evidence["live_captions_assessed"] == "true"
    assert live_captions_evidence["live_captions_basis_recorded"] == "true"
    prerec_ad_evidence = rows["fb.a11y.media.prerecorded_audio_description_seed"]["evidence"][0]["values"]
    assert prerec_ad_evidence["prerecorded_audio_description_scope_declared"] == "true"
    assert prerec_ad_evidence["prerecorded_audio_description_assessed"] == "true"
    assert prerec_ad_evidence["prerecorded_audio_description_basis_recorded"] == "true"
    meaningful_sequence_evidence = rows["fb.a11y.sequence.meaningful_sequence_seed"]["evidence"][0]["values"]
    assert meaningful_sequence_evidence["meaningful_sequence_scope_declared"] == "true"
    assert meaningful_sequence_evidence["meaningful_sequence_assessed"] == "true"
    assert meaningful_sequence_evidence["meaningful_sequence_basis_recorded"] == "true"
    error_suggestion_evidence = rows["fb.a11y.forms.error_suggestion_seed"]["evidence"][0]["values"]
    assert error_suggestion_evidence["error_suggestion_scope_declared"] == "true"
    assert error_suggestion_evidence["error_suggestion_assessed"] == "true"
    assert error_suggestion_evidence["error_suggestion_basis_recorded"] == "true"
    error_prevention_evidence = rows[
        "fb.a11y.forms.error_prevention_legal_financial_data_seed"
    ]["evidence"][0]["values"]
    assert error_prevention_evidence["error_prevention_scope_declared"] == "true"
    assert error_prevention_evidence["error_prevention_assessed"] == "true"
    assert error_prevention_evidence["error_prevention_basis_recorded"] == "true"
    multiple_ways_evidence = rows["fb.a11y.navigation.multiple_ways_seed"]["evidence"][0]["values"]
    assert multiple_ways_evidence["multiple_ways_scope_declared"] == "true"
    assert multiple_ways_evidence["multiple_ways_assessed"] == "true"
    assert multiple_ways_evidence["multiple_ways_basis_recorded"] == "true"
    consistent_nav_evidence = rows["fb.a11y.navigation.consistent_navigation_seed"]["evidence"][0]["values"]
    assert consistent_nav_evidence["consistent_navigation_scope_declared"] == "true"
    assert consistent_nav_evidence["consistent_navigation_assessed"] == "true"
    assert consistent_nav_evidence["consistent_navigation_basis_recorded"] == "true"


def test_engine_verifier_focus_visible_seed_uses_css_focus_and_outline_signals(tmp_path: Path) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "focus.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Focus</title></head><body>"
            "<main><a href='#x'>Link</a><input aria-label='Name'></main>"
            "</body></html>"
        ),
    )
    css_pass = _write(
        tmp_path / "focus-pass.css",
        "a:focus,input:focus{outline:2px solid #000}",
    )
    css_warn = _write(
        tmp_path / "focus-warn.css",
        "a,input{outline:none}",
    )

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Focus")
    pass_report = engine.verify_accessibility_artifacts(str(html), str(css_pass), mode="error")
    warn_report = engine.verify_accessibility_artifacts(str(html), str(css_warn), mode="error")

    pass_row = next(
        f for f in pass_report["findings"] if f["rule_id"] == "fb.a11y.focus.visible_seed"
    )
    warn_row = next(
        f for f in warn_report["findings"] if f["rule_id"] == "fb.a11y.focus.visible_seed"
    )
    assert pass_row["verdict"] == "pass"
    assert warn_row["verdict"] == "warn"


def test_engine_verifier_keyboard_seed_warns_on_pointer_only_custom_click_handlers(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "keyboard-pointer-only.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Keyboard</title></head><body>"
            "<main><div onclick='doThing()'>Open Panel</div></main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "keyboard.css", "")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Keyboard")
    report = engine.verify_accessibility_artifacts(
        str(html),
        str(css),
        profile="strict",
        mode="error",
        claim_evidence={"wcag20": {"keyboard_assessed": True, "keyboard_basis_recorded": True}},
    )

    row = next(
        f for f in report["findings"] if f["rule_id"] == "fb.a11y.keyboard.operable_seed"
    )
    assert row["verdict"] == "warn"
    evidence = row["evidence"][0]["values"]
    assert evidence["interactive_keyboard_target_count"] == "0"
    assert evidence["custom_click_handler_count"] == "1"
    assert evidence["pointer_only_click_handler_count"] == "1"


def test_engine_verifier_focus_order_seed_warns_on_positive_tabindex_and_passes_without_it(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html_warn = _write(
        tmp_path / "focus-order-warn.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Focus Order</title></head><body>"
            "<main><a href='#x' tabindex='2'>Link</a><input aria-label='Name'></main>"
            "</body></html>"
        ),
    )
    html_pass = _write(
        tmp_path / "focus-order-pass.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Focus Order</title></head><body>"
            "<main><a href='#x'>Link</a><input aria-label='Name'></main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "focus-order.css", "body{}")

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Focus Order")
    warn_report = engine.verify_accessibility_artifacts(str(html_warn), str(css), mode="error")
    pass_report = engine.verify_accessibility_artifacts(str(html_pass), str(css), mode="error")

    warn_row = next(
        f for f in warn_report["findings"] if f["rule_id"] == "fb.a11y.focus.order_seed"
    )
    pass_row = next(
        f for f in pass_report["findings"] if f["rule_id"] == "fb.a11y.focus.order_seed"
    )
    assert warn_row["verdict"] == "warn"
    assert pass_row["verdict"] == "pass"
    assert warn_row["evidence"][0]["values"]["positive_tabindex_count"] == "1"
    assert pass_row["evidence"][0]["values"]["positive_tabindex_count"] == "0"
    assert pass_row["evidence"][0]["values"]["interactive_focus_target_count"] == "2"


def test_engine_verifier_bridges_a11y_contract_diagnostics_and_dedups_aggregate_rules(
    tmp_path: Path,
) -> None:
    _require_pdf_engine()

    html = _write(
        tmp_path / "bridge.html",
        (
            "<!doctype html><html lang='en-US'><head><title>Bridge</title></head><body>"
            "<main><h2></h2><label> </label><section role='region'></section></main>"
            "</body></html>"
        ),
    )
    css = _write(tmp_path / "bridge.css", "body{}")
    a11y_report = {
        "ok": False,
        "diagnostics": [
            {"code": "HEADING_EMPTY", "severity": "warning", "message": "Heading empty", "path": "/main/h2[1]"},
            {"code": "LABEL_EMPTY", "severity": "warning", "message": "Label empty", "path": "/main/label[1]"},
            {"code": "REGION_UNLABELED", "severity": "warning", "message": "Region unlabeled", "path": "/main/section[1]"},
        ],
    }

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Bridge")
    report = engine.verify_accessibility_artifacts(
        str(html),
        str(css),
        mode="error",
        a11y_report=a11y_report,
    )

    rows = [
        f
        for f in report["findings"]
        if f["rule_id"] == "fb.a11y.headings_labels.present_nonempty"
    ]
    assert len(rows) == 1
    row = rows[0]
    assert row["verdict"] == "fail"
    assert row["stage"] == "post-emit"
    assert any(
        ev.get("values", {}).get("correlated_origin_stage") == "pre-render"
        for ev in row.get("evidence", [])
    )
    assert any(
        ev.get("values", {}).get("correlation_role") == "summary"
        for ev in row.get("evidence", [])
    )
    assert report["observability"]["dedup_event_count"] >= 1
    assert report["observability"]["correlated_finding_count"] >= 1
    assert report["observability"]["original_source_counts"]["a11y_contract"] >= 1
    assert any(
        item["rule_id"] == "fb.a11y.headings_labels.present_nonempty"
        and item["canonical_stage"] == "post-emit"
        and item["merged_pre_render_count"] >= 1
        for item in report["observability"]["correlation_index"]
    )


def test_engine_verifier_warns_on_low_render_contrast_seed(tmp_path: Path) -> None:
    _require_pdf_engine()

    html_text = (
        "<!doctype html><html lang='en-US'><head><title>Contrast</title></head><body>"
        "<main><p class='c'>Low Contrast Sample</p></main></body></html>"
    )
    css_text = (
        "body{margin:0;background:#fff}"
        "main{padding:24px}"
        ".c{margin:0;font:700 48px Helvetica, Arial, sans-serif;color:#999}"
    )
    html = _write(tmp_path / "contrast.html", html_text)
    css = _write(tmp_path / "contrast.css", css_text)

    engine = fullbleed.PdfEngine(document_lang="en-US", document_title="Contrast")
    png_path = _render_preview_png(engine, html_text, css_text, tmp_path, stem="contrast")
    report = engine.verify_accessibility_artifacts(
        str(html),
        str(css),
        mode="error",
        render_preview_png_path=str(png_path),
    )

    rows = [f for f in report["findings"] if f["rule_id"] == "fb.a11y.contrast.minimum_render_seed"]
    assert rows
    assert any(f["verdict"] in {"warn", "manual_needed"} for f in rows)
    assert any(
        f["evidence"][0]["values"]["render_preview_png_path"] == str(png_path)
        for f in rows
    )
